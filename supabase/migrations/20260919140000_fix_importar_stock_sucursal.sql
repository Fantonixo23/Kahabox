-- =============================================================================
-- Kahabox — Migración 0024: validar la sucursal destino en importar_stock
--
-- La web mandaba a la RPC el UUID "placeholder" de la demo cuando el estado
-- del diálogo quedaba con un id no reconciliado, y cada fila fallaba con
-- `violates foreign key constraint "stock_tienda_sucursal_id_fkey"`.
--
-- Ahora:
--   * p_sucursal_id nulo -> se resuelve a la primera sucursal del comercio.
--   * p_sucursal_id dado -> debe existir Y pertenecer al tenant, si no se
--     aborta con un mensaje claro (no ruido de FK por fila).
-- =============================================================================
...
begin
  ...
  if p_sucursal_id is null then
    ...
    raise exception 'La sucursal destino no pertenece a tu comercio. Recargá la página y volvé a importar.';
  end if;
