...
Implementa los endpoints documentados en
"ICP-Integración CAJA POS - Android 2.0" (REST v1.5.0), para probar la
integración sin terminal físico. Solo librería estándar de Python.

Endpoints:
  /pos/eco                 verificación de conexión
  /pos/venta-ux            venta contado / cuotas
  /pos/venta/debito        venta forzado débito
  /pos/venta/credito       venta forzado crédito
  /pos/descuento           envío de monto (paso 2 de venta-ux/debito/credito)
  /pos/venta-qr            venta QR (con vuelto y promotions opcionales)
  /pos/venta-qr-pix        venta QR PIX
  /pos/extraccion-qr       extracción QR
  /pos/venta-canje         venta por canje de puntos
  /pos/venta-canje-qr      venta por canje de puntos vía QR
  /pos/venta-billetera     venta con billetera electrónica
  /pos/consulta-anulacion  consulta de boletas para anular
  /pos/anulacion           anulación de una boleta

Uso:
    python pos_simulator.py --port 9000
    python pos_simulator.py --port 9000 --interactive
    python pos_simulator.py --port 9000 --delay-cliente 6
    python pos_simulator.py --port 9000 --random --fail-rate 0.3

Con --interactive cada venta pide aprobar/rechazar/timeout por consola,
igual que un cajero interactuando con la terminal real. El simulador
responde las cabeceras CORS (a diferencia de un terminal físico), por lo
que se puede probar desde el navegador sin extensión.
"""
import argparse
import json
import random
...
TARJETAS_CREDITO = [
    {"issuerId": "VC", "nombreTarjeta": "VISA - CLASICA - BANCO ITAU PY"},
    {"issuerId": "MC", "nombreTarjeta": "MASTERCARD - BLACK - BANCO ITAU PY"},
    {"issuerId": "CB", "nombreTarjeta": "CABAL - CLASICA - BANCO FAMILIAR"},
    {"issuerId": "AC", "nombreTarjeta": "AMERICAN EXPRESS - BANCO GNB"},
    {"issuerId": "BC", "nombreTarjeta": "BANCARD - CLASICA"},
    {"issuerId": "CP", "nombreTarjeta": "PANAL - CLASICA"},
    {"issuerId": "PC", "nombreTarjeta": "CREDICARD - CLASICA"},
    {"issuerId": "TC", "nombreTarjeta": "TARJETA CREDITO - BANCO REGIONAL"},
    {"issuerId": "DC", "nombreTarjeta": "DINERS CLUB - INTERNATIONAL"},
]
TARJETAS_DEBITO = [
    {"issuerId": "VD", "nombreTarjeta": "VISA - PREPAGA - BANCO ITAU PY"},
    {"issuerId": "MD", "nombreTarjeta": "MASTERCARD - DEBITO - BANCO CONTINENTAL"},
    {"issuerId": "ID", "nombreTarjeta": "INFONET - DEBITO"},
    {"issuerId": "TD", "nombreTarjeta": "TARJETA DEBITO - BANCO REGIONAL"},
    {"issuerId": "UD", "nombreTarjeta": "UNICA - DEBITO"},
]
BILLETERAS = {
...
class Estado:
    def __init__(self, interactivo, delay, delay_cliente, random_outcomes=False,
        ...
        self.delay = delay
        self.delay_cliente = delay_cliente
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/status"):
            ...
            self._responder(200, {"transacciones_abiertas": abiertas, "boletas_registradas": boletas})
        else:
    ...
    def do_POST(self):
        ...
        if estado.delay_cliente and path != "/pos/eco":
            ...
            if hi > 0:
                ...
                estado.registrar(f"{path} -> simulando {espera:.1f}s de cliente...")
                time.sleep(espera)
        ...
        if body is None:
            ...
            try:
                ...
                estado.registrar(f"{path} -> TIMEOUT simulado")
    ...
    def _eco(self, body, estado):
        ...
        if estado.random_outcomes and random.random() < estado.eco_fail_rate:
            estado.registrar("/pos/eco -> FALLO (POS no responde)")
    ...
    def _emitir_venta(self, estado, factura, monto, tipo, cuotas=0, plan=0):
        ...
        bin_ = tarjeta["issuerId"] + rand_digits(8)
        nsu = rand_digits(6)
        with estado.lock:
    ...
    def _venta_qr(self, body, estado):
        ...
        tarjeta = random.choice(TARJETAS_CREDITO + TARJETAS_DEBITO)
        nro_boleta = rand_digits(12)
        estado.registrar_boleta(mask_bin(tarjeta["issuerId"] + rand_digits(15)), nro_boleta, body["monto"], "QR")
        return 200, {
            "codigoAutorizacion": rand_digits(6),
            "codigoComercio": rand_digits(7),
            "issuerId": tarjeta["issuerId"],
    ...
    def _venta_qr_pix(self, body, estado):
        ...
        resumen = f"VENTA QR PIX | factura={body['facturaNro']} monto={body['monto']}"
        rechazo = self._pedir_aprobacion(estado, resumen)
        if rechazo:
            return error_400(rechazo)
    ...
    def _extraccion_qr(self, body, estado):
        ...
        nro_boleta = rand_digits(12)
        estado.registrar_boleta(mask_bin(tarjeta["issuerId"] + rand_digits(9)), nro_boleta, monto, "Extraccion QR")
        return 200, {
            "codigoAutorizacion": rand_digits(6),
            "codigoComercio": rand_digits(7),
            "issuerId": tarjeta["issuerId"],
    ...
    def _venta_billetera(self, body, estado):
        ...
        resumen = f"VENTA BILLETERA {billetera} | factura={body['facturaNro']} monto={body['monto']}"
        rechazo = self._pedir_aprobacion(estado, resumen)
        if rechazo:
            return error_400(rechazo)
        nro_boleta = rand_digits(12)
    ...
    def _consulta_anulacion(self, body, estado):
        ...
        return 200, {"listado": [{k: v for k, v in b.items() if k != "anulada"} for b in listado]}
...
def main():
    ap = argparse.ArgumentParser(description="Simulador POS CajaPOS Bancard (API REST v1.5.0)")
    ap.add_argument("--port", type=int, default=9000)
    ap.add_argument("--host", default="0.0.0.0")
    ap.add_argument("--interactive", action="store_true", help="Pedir aprobar/rechazar/timeout cada venta por consola")
    ap.add_argument("--delay", type=float, default=0.0, help="Segundos de demora simulada por request (toda ruta)")
    ap.add_argument("--delay-cliente", type=parse_rango_segundos, default=(0.0, 0.0),
                    help="Segundos de demora simulando al cliente en el POS ('6' o '30-60')")
    ap.add_argument("--random", dest="random_outcomes", action="store_true",
                    help="Resultados aleatorios: rechazos, timeouts y fallos de eco")
    ap.add_argument("--fail-rate", type=float, default=0.15, help="Probabilidad de rechazo con --random")
    ap.add_argument("--timeout-rate", type=float, default=0.03, help="Probabilidad de timeout con --random")
    ap.add_argument("--eco-fail-rate", type=float, default=0.05, help="Probabilidad de que falle el eco con --random")
    args = ap.parse_args()
    ...
    if args.interactive:
        ...
        print(f"Modo: AUTOMATICO ALEATORIO (fail_rate={args.fail_rate}, timeout_rate={args.timeout_rate}, eco_fail_rate={args.eco_fail_rate})")
    else:
