...
**Persistencia de la demo en localStorage**: el mock (`mock.ts`) guarda su estado en `localStorage` (clave `kahabox_demo_v1`) para que los productos cargados sobrevivan recargas — pero eso es **por dispositivo**. La sincronización en vivo entre el celular y la compu la hace el relay; al conectar Supabase el escáner remoto pasa automáticamente a **Supabase Realtime** (broadcast + presence, canal por `tenant_id` + caja) y este transporte de demo queda solo como fallback cuando no hay `.env`.
